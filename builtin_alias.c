 ...
 *
 * This function can be used to perform
 * various operations related to shell aliases,
 * including printing all aliases, printing a specific alias,
 * or setting a new alias.
 *
...
/**
 * set_alias - Manage aliases by either creating a
 *new one or updating an existing one.
 *
 * This function sets an alias with a given name and
 * value. If an alias with the
 * same name already exists, its value is updated;
 * otherwise, a new alias is created.
 *
...
void set_alias(char *var_name, char *value)
{
	...
	*value = '\0';
	...
	for (j = 0, k = 0; value[j]; j++)
	{
		...
		if (value[j] != '\'' && value[j] != '"')new_value[k++] = value[j];
	}
	new_value[k] = '\0';
	while (temp)
	{
 ...
 *
 * This function iterates through the array of arguments,
 *checks for aliases in a linked list,
 * and if a match is found, replaces the alias with
 * its associated value. It ensures proper memory
